...
void parseArg(int argc, char **argv, int &bnd) {
  if (argc != 2) {
    std::cout << "   BMC for two FIFO designs (w./w.o. overflow & underflow "
                 "protection)."
              << std::endl;
    std::cout << "   Checking if overflow/underflow is possible.               "
                 "         "
              << std::endl;
    std::cout << "   Usage:  " << argv[0] << " bound (a positive integer)"
              << std::endl;
    exit(1);
  }
  try {
