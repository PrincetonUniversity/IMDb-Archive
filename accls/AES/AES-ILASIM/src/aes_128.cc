...
#include <cassert>
auto rcon_mem = MemConst(0, std::map<int, int>(), 4, 8);
auto S_mem = MemConst(0, std::map<int, int>(), 8, 8);
auto xS_mem = MemConst(0, std::map<int, int>(), 8, 8);
...
AES_128::AES_128() : model("AES_128_Rnd") {
  std::map<int, int> rcon_map;
  for (int i = 0; i < 10; i++)
    rcon_map[i] = rcon_table[i];
  rcon_mem = MemConst(0, rcon_map, 4, 8);

  std::map<int, int> S_map;
  for (int i = 0; i < 256; i++)
    S_map[i] = S_table[i];
  S_mem = MemConst(0, S_map, 8, 8);
 
  std::map<int, int> xS_map;
  for (int i = 0; i < 256; i++)
    xS_map[i] = xS_table[i];
  xS_mem = MemConst(0, xS_map, 8, 8);
 
 
 
  auto plaintext = model.NewBvInput("plaintext", 128);
  auto key_in = model.NewBvInput("key_in", 128);
  auto plaintext_cache = model.NewBvState("plaintext_cache", 128);
  ...
  { // Midround
    ...
    instr.SetUpdate(round, round + 1);
    instr.SetUpdate(plaintext_cache, plaintext);
  }
...

  {
    ...
    instr.SetDecode((round == 11) & (plaintext_cache != plaintext));

    instr.SetUpdate(round, BvConst(0, 4));
  }
