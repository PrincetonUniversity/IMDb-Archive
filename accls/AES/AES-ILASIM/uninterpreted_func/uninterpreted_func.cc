...
sc_biguint<2> AES::unknown1() {
  return 0;
}

sc_biguint<128> AES::unknown2() {
  return 0;
}

sc_biguint<128> AES::aes128(sc_biguint<128> arg_0, sc_biguint<128> arg_1) {
  uint32_t output[4] = {0, 0, 0, 0};
  uint32_t key[4] = {0, 0, 0, 0};
  for (int i = 0; i < 4; i++) {
    for (int j = 0; j < 4; j++) {
      uint32_t tmp = arg_0.range(i * 32 + j * 8 + 7, i * 32 + j * 8).to_uint();
      output[3 - i] += (tmp << ((3 - j) * 8)); 
      tmp = arg_1.range(i * 32 + j * 8 + 7, i * 32 + j * 8).to_uint();
      key[3 - i] += (tmp << ((3 - j) * 8));
    }
  }
  ...
  for (int i = 0; i < 4; i++) {
    for (int j = 0; j < 4; j++) {
      ...
      tmp = tmp >> (j * 8);
      tmp = tmp & (0xff);
      result.range((3-i)*32 + (3-j)*8 + 7, (3-i)*32 + (3-j)*8) = tmp;
    }
  }
  // cout << "arg_0:" << arg_0.to_string(SC_HEX) << endl;
  // cout << "arg_1:" << arg_1.to_string(SC_HEX) << endl;
  // cout << "result:" << result.to_string(SC_HEX) << endl;
  return result;
}
