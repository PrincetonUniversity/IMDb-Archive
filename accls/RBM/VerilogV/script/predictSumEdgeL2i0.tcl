...
assume { ( cycles[2] |-> 1 ) }
assume { ( cycles[3] |-> 1 ) }
assume { ( cycles[4] |-> acc_0.state_rbm_0_cmos32soi_rbm_predict_rbm[21] ) }
assume { ( cycles[5] |-> acc_0.state_rbm_0_cmos32soi_rbm_predict_rbm[22] ) }
assume { ( cycles[6] |-> ( acc_0.state_rbm_0_cmos32soi_rbm_predict_rbm[23] && acc_0.state_rbm_0_cmos32soi_rbm_predict_rbm_next[22] )) }
...
# pre-match
assume { (cycles[4] |-> ( GM.sumOfpow2 == acc_0.mux_sumOfpow2_ln434_z ) ) }
assume { (cycles[3] |-> ( GM.predict_max == acc_0.mux_predict_vector_ln471_0_q[386:371] ) ) }
assume { (cycles[4] |-> ( GM.jstate == acc_0.mux_j_ln434_z ) ) } 
assume { (cycles[4] |-> ( acc_0.mux_j_ln434_z < 5) ) }
assume { (cycles[3] |-> ( acc_0.memwrite_pow2_ln455_q == mem_pow2.mem) ) }
...
# load_info
assume { ( cycles[5] |->  ( reg0 == acc_0.mux_j_ln434_z ) )   }
# final-check
...
assert { ( last_cycle |-> ( acc_0.mux_j_ln434_z <= 5) ) }
