...
assume -name {aux-4} \
{
    acc_0.memwrite_rbm_visible_unit_ln365_q[79] == (acc_0.mux_j_ln396_z < 5)
}
...
# sequence
assume { ( cycles[0] |-> 1 ) }
assume { ( cycles[1] |-> acc_0.state_rbm_0_cmos32soi_rbm_predict_rbm[32] ) }
...
assume { ( cycles[6] |-> ( acc_0.state_rbm_0_cmos32soi_rbm_predict_rbm[16] && acc_0.state_rbm_0_cmos32soi_rbm_predict_rbm_next[15] )) }
...
assume { (cycles[0] |-> ( GM.jstate == acc_0.mux_j_ln396_z + 1 ) ) } 
...
assume { ( cycles[6] |->  ( reg2 == acc_0.mux_sum_ln400_z ) ) }
assume { ( cycles[6] |->  ( reg3 == acc_0.mux_predict_vector_ln471_0_q[386:371] ) ) }
# final-check
assert { ( finish |-> ( GM.predict_v_cnt == reg0 ) ) }
assert { ( finish |-> ( (GM.predict_h_cnt == reg1) || (GM.predict_h_cnt == 0 && reg1 == acc_0.num_hidden +1 ) ) ) }
